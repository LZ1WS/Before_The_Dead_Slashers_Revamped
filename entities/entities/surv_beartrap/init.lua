...
local function IsSurvivor(ent) 
	if ent:Team() == TEAM_SURVIVORS then return true end -- MODIFY
	return false
end
...
local function ActivateOnPlayer(ent, self)
	...
	self:ResetSequence(self:LookupSequence("close")) -- Play animation
	timer.Simple(self:SequenceDuration(), function()
	self:SetSequence(self:LookupSequence("closed")) -- Play animation
	end)
	RefreshTap(ent, self)
...
function ENT:Initialize()
	...
	self:SetModel("models/props/mj_dbd/functional_props/mj_dbd_beartrap.mdl")
...
function ENT:Touch(ent)
	if ent.beartrap or self.isused == 1 or !ent:IsPlayer() or IsSurvivor(ent) then return end
