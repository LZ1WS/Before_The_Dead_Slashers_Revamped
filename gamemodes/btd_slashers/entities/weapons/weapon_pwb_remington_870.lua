...
killicon.Add( "weapon_pwb_remington_870", "pwb/sprites/remington_870", Color( 255, 255, 255, 255 ) )
end
...
SWEP.ViewModel = "models/pwb/weapons/v_remington_870.mdl"
...
SWEP.Primary.Sound = Sound( "Weapon_PWB_Remington_870.Shoot" )
...
surface.DrawLine( x - 40, y, x - 36, y )
surface.DrawLine( x + 40, y, x + 36, y )
surface.DrawLine( x, y - 40, x, y - 36 )
surface.DrawLine( x, y + 40, x, y + 36 )
end
end
...
self.BobTimer = CurTime()
self.Reloading = 1
self.ReloadingTimer = CurTime() + 0.7
self.Idle = 0
self.IdleTimer = CurTime() + self.Owner:GetViewModel():SequenceDuration()
self.Owner:SetFOV( 0, self.IronOutTime )
self.Owner:SetWalkSpeed( 200 )
...
function SWEP:Think()
self:StopSound( "Weapon_Shotgun.Special1" )
self.BobScale = self.Weapon:GetNWString( "BobSway", 0.75 )
self.SwayScale = self.Weapon:GetNWString( "BobSway", 0.75 )
if self.Reloading == 1 then
if self.ReloadingTimer < CurTime() + 0.7 and self.ReloadingTimer > CurTime() + 0.5 then
