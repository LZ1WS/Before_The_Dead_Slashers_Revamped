...
if IsValid(BackGroundPanel) then return end
	...
	img_charac = vgui.Create( "DImage", PersoPANEL )
	if isstring(ImageCharac) then
	img_charac:SetImage( ImageCharac )
	else
	img_charac:SetMaterial( ImageCharac )
	end
...
if (CharacName == "Steve Harrington") then
	ShowPlayerScreen(TeamName,TeamText,CharacName,CharacText,"icons/steve.png",0)
else
