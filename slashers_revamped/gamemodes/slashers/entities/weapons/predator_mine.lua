...
SWEP.ViewModel = "models/viewmodels/c_pred_lasermine.mdl"
SWEP.WorldModel = "models/predatorremotemine.mdl"
SWEP.ShowViewModel = true
SWEP.ShowWorldModel = false

SWEP.WElements = {
	["mine"] = { type = "Model", model = "models/predatorremotemine.mdl", bone = "ValveBiped.Bip01_R_Hand", rel = "", pos = Vector(3.052, 2.591, -0.21), angle = Angle(19.947, 128.865, 97.194), size = Vector(0.899, 0.899, 0.899), color = Color(255, 255, 255, 255), surpresslightning = false, material = "", skin = 0, bodygroup = {} }
}
...
SWEP.Secondary.Sound		= "Weapon_Crowbar.Single"	
SWEP.Primary.Damage	= 100 
SWEP.Primary.NumShots = 1
SWEP.Primary.Cone = 0
...
function SWEP:Think()
 ...
 if self.Owner.Cloak == true then
	...
	self.WElements["mine"].color = Color(255,255,255,3)	
	self.WElements["mine"].material = "sprites/heatwave"
	end
	self.Weapon.Cloak = true
	else
	if self.Weapon.Cloak == true then	
	self.WElements["mine"].color = Color(255,255,255,255)	
	self.WElements["mine"].material = "models/glass"	
	end
...
function SWEP:SecondaryAttack()
	...
	self.Owner.Minesnumber = self.Owner.Minesnumber - 1	
	net.Start( 'MinesGet' )
	net.WriteFloat( self.Owner.Minesnumber )	
	net.Send( self.Owner )			
	...
	if self.Owner.Minesnumber > 0 then
	timer.Simple( 0.84, function() 
		self:SendWeaponAnim(ACT_VM_DRAW)	
	end)	
	else
	timer.Simple( 0.84, function()
	self.Owner:StripWeapon( "predator_mine" )
	end)
	end
end
