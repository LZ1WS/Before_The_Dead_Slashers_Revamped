...
function ENT:Use( activator )
	...
	if activator:IsPlayer() and activator:HasWeapon( 'hillbilly_chainsaw' ) then
		...
		--activator:GetWeapon( 'hillbilly_chainsaw' ):TestCollig(self:GetClass())
		...
		if chainswep.AddonsArray[chainswep:GetAddonSlot()] != NULL then
		...
		chainswep:SetBonusSteer(chainswep:GetBonusSteer() + chainswep.AddonsStats[pickedAddon][1])					
		...
		self:EmitSound("weapons/item_pickup.ogg")
